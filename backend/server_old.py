...
# ===============================
# AUTHENTICATION ROUTES - MOVED TO routes/auth_routes.py
# ===============================
...
@api_router.post("/bulk-import/customers/upload")
async def upload_customers_endpoint(
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if existing:
                    results["skipped"].append({"row": index + 2, "isletme_id": isletme_id, "reason": "Bu işletme ID zaten mevcut"})
                    continue
...
@api_router.post("/bulk-import/products/template")
async def download_product_template_endpoint(
...
@api_router.post("/bulk-import/products/upload")
async def upload_products_endpoint(
    file: UploadFile = File(...),
    current_user: User = Depends(require_role([UserRole.SALES_REP, UserRole.ADMIN, UserRole.WAREHOUSE_MANAGER]))
):
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if existing:
                    results["skipped"].append({"row": index + 2, "urun_id": urun_id, "reason": "Bu ürün ID zaten mevcut"})
                    continue
                ...
                results["success"].append({"row": index + 2, "urun_id": urun_id, "urun_adi": product['name'], "stok": int(row['stok'])})
...
@api_router.post("/bulk-import/orders/template")
async def download_order_template_endpoint(
    current_user: User = Depends(require_role([UserRole.SALES_REP, UserRole.ADMIN]))
):
    """Sipariş Excel template"""
    return {
        ...
        "example": {
            ...
            "siparis_durumu": "delivered"
        },
        "note": "En fazla 10 ürün eklenebilir (urun_id_1 ile urun_id_10 arası)"
    }

@api_router.post("/bulk-import/orders/upload")
async def upload_orders_endpoint(
    file: UploadFile = File(...),
    current_user: User = Depends(require_role([UserRole.SALES_REP, UserRole.ADMIN]))
):
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                if existing:
                    results["skipped"].append({"row": index + 2, "siparis_id": siparis_id, "reason": "Bu sipariş ID zaten mevcut"})
                    continue
                
                customer = await db.users.find_one({"customer_number": musteri_id})
                if not customer:
                    results["errors"].append({"row": index + 2, "error": f"Müşteri ID bulunamadı: {musteri_id}"})
                    continue
                ...
                if not products:
                    results["errors"].append({"row": index + 2, "error": "Geçerli ürün bulunamadı"})
                    continue
